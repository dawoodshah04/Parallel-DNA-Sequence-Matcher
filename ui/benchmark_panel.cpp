...
static double run_sw_opencl(int seq_len, int db_size, int& out_score,
    ...
    cl_kernel sw_kern = clCreateKernel(prog, "sw_wavefront", &err);
    ...
    for (int i = 0; i < db_size; ++i) {
        ...
        // Accumulate GPU time: first wavefront start → find_max end
        cl_ulong t0 = 0, t1 = 0;
...
static void draw_bar_chart(const std::vector<BenchmarkEntry>& entries,
    ...
    constexpr float BAR_H    = 22.0f;
    constexpr float BAR_PAD  = 8.0f;
    constexpr float LABEL_W  = 110.0f;

    ImVec4 colors[] = {
        {0.30f, 0.60f, 0.90f, 1.0f},   // sequential — blue
        {0.20f, 0.75f, 0.45f, 1.0f},   // omp 2t — green
        {0.15f, 0.65f, 0.35f, 1.0f},   // omp 4t
        {0.10f, 0.55f, 0.25f, 1.0f},   // omp 8t
        {0.90f, 0.55f, 0.15f, 1.0f},   // MPI — orange
        {0.70f, 0.30f, 0.90f, 1.0f},   // OpenCL — purple
    };
    ...
    for (std::size_t idx = 0; idx < entries.size(); ++idx) {
        ...
        if (!e.measured) {
            ImGui::SetCursorScreenPos(ImVec2(cursor.x + LABEL_W, y));
            if (!e.error.empty()) {
        ...
        float bx1 = bx0 + std::max(bar_w, 2.0f);

        dl->AddRectFilled(ImVec2(bx0, y + 2), ImVec2(bx1, y + BAR_H),
                          col, 3.0f);
        ...
        dl->AddText(ImVec2(bx1 + 6, y + 4), IM_COL32_WHITE, info);
    }

    // Advance cursor past the bars
    ImGui::Dummy(ImVec2(avail_w + LABEL_W,
                        static_cast<float>(entries.size()) * (BAR_H + BAR_PAD)));
}
...
void render_benchmark_panel(BenchmarkState& state) {
    ...
    ImGui::SliderInt("Sequence length (bp)", &state.seq_length, 50, 1000);
    ImGui::SliderInt("DB size (# sequences)", &state.db_size, 5, 200);
    ImGui::SliderInt("MPI ranks",             &state.mpi_ranks, 1, 8);
    ImGui::Spacing();

    bool run = ImGui::Button("Run Benchmark", ImVec2(160, 0));
    ImGui::SameLine();
    ...
    if (state.entries[0].measured) {
        ImGui::Spacing();
        ImGui::SeparatorText("Speedup Table");
        if (ImGui::BeginTable("speedup_tbl", 3,
            ...
            for (auto& e : state.entries) {
                ImGui::TableNextRow();
                ImGui::TableSetColumnIndex(0); ImGui::Text("%s", e.label);
                ...
                if (e.measured && base_ms > 0.0)
                    ImGui::Text("%.2fx", base_ms / e.time_ms);
                else
                    ImGui::TextDisabled("—");
            }
            ImGui::EndTable();
